        ...
        <div class="right_c">
            <el-card shadow="never" style="text-align: left;margin: 2px auto;">
                <div slot="header" class="clearfix">
                    <span>文章列表</span>
                    <el-button v-if="search_key" @click="setFilters()" style="float: right; padding: 3px 0" type="text">重置条件<el-tag type="warning" size="mini">{{search_key}}</el-tag> </el-button>
                </div>
                <el-card shadow="never" style="text-align: left;margin: 10px auto;"  v-for="item in show_list" :key="item">
                    <div slot="header" class="clearfix">
                        <span><i class="el-icon-document"></i>{{item.title}}</span>
                        <el-button  @click="findArticle(item.key)" style="float: right; padding: 3px 0" type="text">访问</el-button>
                    </div>
                    <div>
            ...
            <el-card shadow="never" style="text-align: left;margin: 2px auto;">
                <el-pagination background layout="prev, pager, next" :total="page_total" :page-size="page_size" :current-page="page_index" @current-change="pageChange">
                </el-pagination>      
            </el-card>    
...
export default {
  ...
  methods: {
    ...
    initList(){
        ...
        this.$data.show_list=[];
        for(var i=(page_size*(page_index-1));i<page_size*page_index && i<this.$data.page_total;i++){
